<?php
    ...
    $categorys = $conn->query($sql2)->fetchAll();
    $checkAdmin = false;
    $userId = "";
    if(!empty($_COOKIE)){
    ...
    function filterProduct($products, $category){
        $filterProducts = array_filter($products, function ($product) use ($category) {
    ...
    if($_SERVER['REQUEST_METHOD'] === 'GET'){
        ...
        if (isset($_GET['category']) && $category !== '') {
            $resultProducts = filterProduct($products, $category);
        } else {
...
<body>
    <div class='container'>
        ...
        <section class="conten">
            <div class='heder'>
                <h1>Sản phẩm</h1>     
                <form method="GET" class='ok' >
                    <select class="form-select" aria-label="Default select example" name='category'>
                        <option selected value=''>Tất cả các loại</option>
