<?php
    ...
    if($_SERVER['REQUEST_METHOD'] === 'POST'){
        ...
        $file = $_FILES['img'];
        $img = $file['name'] ? $file['name'] : $product['image'];    
        ...
        if(empty($errors)){
            ...
            $sql = "UPDATE product SET product_name='$productname', description='$description', price='$price', quantity='$quantity', image='$img', category_id='$category' WHERE product_id = $idPro";
            $conn->query($sql);
...
<body>
    <div class='boxCreate'>
        <form method="post" enctype='multipart/form-data'>
            <div class="mb-3">
                <input type="text" class="form-control" name="productname" value="<?= $product['product_name'] ?>" placeholder='Nhập vào tên sản phẩm'>
            ...
            <div class="row mb-3">
                <div class="col">
                    <input type="number" class="form-control"  name="price" placeholder="Price" min=0 value="<?= $product['price'] ?>">
                    <span style="color: red;"><?= $errors['l2'] ?? ''?></span>
                </div>
                <div class="col">
                    <input type="number" class="form-control" name="quantity" placeholder="Số lượng" min=0 value="<?= $product['quantity'] ?>">
            ...
            <div class="form-floating mt-3">
                <textarea class="form-control" placeholder="Leave a comment here" name="description" id="floatingTextarea" ><?= $product['description'] ?></textarea>
                <label for="floatingTextarea">Nhập vào mô tả</label>
            </div>
            <span style="color: red;"><?= $errors['l5'] ?? ''?></span>
            <button type="submit" class="btn btn-primary col-12 my-3">Update product</button> 
        </form>
    </div>
</body>
</html>
